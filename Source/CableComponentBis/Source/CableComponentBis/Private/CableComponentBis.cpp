...
public:
	...
	FCableSceneProxy(UCableComponentBis* Component)
...
FPrimitiveSceneProxy* UCableComponentBis::CreateSceneProxy()
{
	return new FCableSceneProxy(this);
}

int32 UCableComponentBis::GetNumMaterials() const
{
	return 1;
}

void UCableComponentBis::OnRegister()
{
	Super::OnRegister();
...
void UCableComponentBis::VerletIntegrate(float InSubstepTime, const FVector& Gravity)
{
...
void UCableComponentBis::PerformSubstep(float InSubstepTime, const FVector& Gravity)
{
...
void UCableComponentBis::SetAttachEndToComponent(USceneComponent* Component, FName SocketName)
{
...
void UCableComponentBis::SetAttachEndTo(AActor* Actor, FName ComponentProperty, FName SocketName)
{
...
AActor* UCableComponentBis::GetAttachedActor() const
{
	return AttachEndTo.OtherActor.Get();
}

USceneComponent* UCableComponentBis::GetAttachedComponent() const
{
	return Cast<USceneComponent>(AttachEndTo.GetComponent(GetOwner()));
}

void UCableComponentBis::GetCableParticleLocations(TArray<FVector>& Locations) const
{
	Locations.Empty();
...
void UCableComponentBis::OnVisibilityChanged()
{
...
FBoxSphereBounds UCableComponentBis::CalcBounds(const FTransform& LocalToWorld) const
{
...
FTransform UCableComponentBis::GetSocketTransform(FName InSocketName, ERelativeTransformSpace TransformSpace) const
{
...
bool UCableComponentBis::HasAnySockets() const
{
	return (Particles.Num() >= 2);
}

bool UCableComponentBis::DoesSocketExist(FName InSocketName) const
{
